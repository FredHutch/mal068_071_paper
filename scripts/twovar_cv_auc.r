...
if (length(args)!=2) {
  stop(paste("Two arguments must be supplied",
             "(analysis data file path, output filename).",
             "provided:", paste(args, collapse = ", ")),
       call.=FALSE)
}
analysis_data_filename <- args[1]
...
mdl_form <- as.formula(paste("y ~ val1 + val2",
                             if_else(length(unique(analysis_data$arm)) > 1, "+ arm", "")))

test_codes <- unique(analysis_data$test_code)
code_combos <- combn(test_codes, 2)
...
results <- foreach (i = 1:ncol(code_combos),
                    ...
                    .packages = c("glmnet", "dplyr", "tidyr", "cvAUC")) %dopar% {
                      ...
                      names(model_data)[4:5] <- paste0("val", 1:2)
