...
dupcor_imm <- duplicateCorrelation(assay(MAL071_rnaseq),
                                   design_imm,
                                   block = MAL071_rnaseq$ptid)

lm_imm <- lmFit(assay(MAL071_rnaseq),
                design_imm,
...
for(i in 1:length(index_visit))
{
  de_genes_imm <- topTable(eb_imm,
                         ...
                         mutate(gene_index = 1:nrow(MAL071_rnaseq)) %>%
                         clean_names
...
gene_names <- as_tibble(rowData(MAL071_rnaseq))
...
for(i in 1:length(index_visit))
{
  cam_imm <- camera(assay(MAL071_rnaseq),
                    index = gene_ids,
                    design = design_imm,
...
### Compute variables
exp_dt <- data.table(assay(MAL071_rnaseq), keep.rownames = "gene_id")
...
meta <- as.data.table(colData(MAL071_rnaseq)[, c("age", "sex", "arm", "infection", "visit_day", "ptid")])
meta$sample_name <- rownames(colData(MAL071_rnaseq))
...
fwrite(imm_downselected_scores, file = here("data/imm_downselected_scores71_all.csv"))
